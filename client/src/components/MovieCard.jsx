...
const MovieCard = ({ movie }) => {
    ...
    return (
        <div className='flex flex-col justify-between p-3 bg-gray-800 rounded-2xl hover:-translate-y-1 transition duration-300 w-62'>
            ...
            <p className="text-sm text-gray-400 mt-2 ">{new Date(movie.release_date).getFullYear()} <Dot className="inline w-4 h-4"/> {movie.genres.slice(0, 2).map(genre => genre.name).join('|')} <Dot  className="inline w-4 h-4"/> {timeFormat(movie.runtime)} </p>
